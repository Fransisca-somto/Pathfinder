...
const startTrip = async (vehicleId, lat, lng) => {
    try {
        ...
        const { error } = await supabase_1.supabase.from('trips').insert({
...
const endTrip = async (vehicleId, lat, lng) => {
    try {
        ...
        const { data: openTrip, error: fetchError } = await supabase_1.supabase
            .from('trips')
            .select('*')
