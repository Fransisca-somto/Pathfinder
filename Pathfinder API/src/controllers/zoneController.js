...
exports.assignVehiclesToZone = exports.getAssignedVehicles = exports.deleteZone = exports.updateZone = exports.createZone = exports.getZones = void 0;
const supabase_1 = require("../config/supabase");
const zoneService_1 = require("../services/zoneService");
// Helper to map DB record to Flutter model
const mapZoneToFlutter = (data) => ({
    id: data.id,
    name: data.name,
    type: data.type,
...
const createZone = async (req, res) => {
    try {
        ...
        await (0, zoneService_1.refreshZoneCache)();
...
const updateZone = async (req, res) => {
    try {
        ...
        if (shape !== undefined)
            updatePayload.shape_type = shape;
        if (radius !== undefined)
            updatePayload.radius_meters = radius;
        if (coordinates !== undefined) {
            if (coordinates.centerLatitude !== undefined)
                updatePayload.center_latitude = coordinates.centerLatitude;
            if (coordinates.centerLongitude !== undefined)
                updatePayload.center_longitude = coordinates.centerLongitude;
            if (coordinates.polygonPoints !== undefined)
                updatePayload.polygon_points = coordinates.polygonPoints;
        }
        const { data, error } = await supabase_1.supabase
            .from('zones')
...
const getAssignedVehicles = async (req, res) => {
    try {
        ...
        const zoneId = req.params.id;
        // Verify ownership
        const { data: existing } = await supabase_1.supabase.from('zones').select('id').eq('id', zoneId).eq('owner_id', ownerId).single();
        if (!existing) {
            res.status(404).json({ error: 'Zone not found' });
            return;
        }
        const { data, error } = await supabase_1.supabase.from('zone_vehicles').select('vehicle_id').eq('zone_id', zoneId);
        if (error)
            throw error;
...
exports.getAssignedVehicles = getAssignedVehicles;
// POST /zones/:id/vehicles — Assign vehicles to zone
const assignVehiclesToZone = async (req, res) => {
    try {
        const ownerId = req.user.id;
        const zoneId = req.params.id;
        ...
        await supabase_1.supabase.from('zone_vehicles').delete().eq('zone_id', zoneId);
        // Insert new assignments
        if (vehicleIds.length > 0) {
            const inserts = vehicleIds.map(vid => ({ zone_id: zoneId, vehicle_id: vid }));
            const { error } = await supabase_1.supabase.from('zone_vehicles').insert(inserts);
            if (error)
                throw error;
        }
        await (0, zoneService_1.refreshZoneCache)();
...
exports.assignVehiclesToZone = assignVehiclesToZone;
